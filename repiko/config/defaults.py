...
from typing import TYPE_CHECKING, TypedDict, is_typeddict, Generator

from repiko.config.meta import ConfigMeta
from repiko.config.dictsub import ConfigDict, UnitDict
from repiko.config.util import isBuiltinType
if TYPE_CHECKING:
...
class TypedDictPattern(DefaultsPattern):
    ...
    def docsGen(cls,config:Config):
        if isinstance(config.data,ConfigDict):
...
class ClassPattern(DefaultsPattern):
    ...
    @classmethod
    def canAccept(cls,pattern):
        return isinstance(pattern,type) and not isBuiltinType(pattern) # pattern 是类

    @classmethod
    def accept(cls,config:Config,pattern:type):
        config._pattern=ConfigMeta.mimic(pattern) # 用 ConfigMeta 包装，dict、pattern 作为基类
        return config._pattern

    @staticmethod
    def _toData(config: Config) -> ConfigDict:
        return config._pattern(patternLocal=config._defaultsLocal)
    ...
    @classmethod
    def updateData(cls,config:Config,data:dict):
        return config._pattern(data, patternLocal=config._defaultsLocal)
        # data=super().updateData(config,data)
        # if not isinstance(data,config._pattern):
        #     data=config._pattern(data)
        # return data
